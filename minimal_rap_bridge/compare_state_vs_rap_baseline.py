#!/usr/bin/env python3
"""Trimmed state-vs-RAP baseline compare with captured or native-policy control."""
...
from render_pufferdrive_to_rap import (
    Drive,
    ScenarioRenderer,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Baseline state BEV vs RAP camera compare")
    parser.add_argument("--out-dir", type=Path, default=Path("/tmp/pd_rap_state_compare_baseline"))
    ...
    parser.add_argument(
        "--no-panels",
        action="store_true",
        help="Skip writing BEV+RAP panel images (metrics CSV only) to isolate panel/OpenCV issues",
    )
    parser.add_argument(
        "--log-every",
        type=int,
        default=1,
        help="Print per-frame progress every N frames",
    )
    parser.add_argument(
        "--control-mode",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Use captured replay baseline or native policy closed-loop stepping",
    )
    parser.add_argument(
        "--policy-path",
        type=str,
...
def box_corners_xy(cx: float, cy: float, length: float, width: float, yaw: float) -> np.ndarray:
    ...
    local = np.array([[half_l, half_w], [half_l, -half_w], [-half_l, -half_w], [-half_l, half_w]], dtype=np.float32)
    c = float(np.cos(yaw))
    s = float(np.sin(yaw))
...
def main() -> None:
    ...
    try:
        ...
        renderer = ScenarioRenderer(camera_channel_list=args.cameras, width=args.render_width, height=args.render_height)
        maybe_rescale_renderer_intrinsics(
            renderer=renderer,
        ...
        for t in range(args.frames):
            ...
            if not args.no_panels:
                bev = draw_bev_panel(scenario=scenario, size=args.bev_size, radius=args.map_radius, camera_yaw=camera_yaw)
                label = (
            ...
            if (t % args.log_every) == 0:
                print(
                    f"[frame {t:04d}] nonzero_{args.panel_camera}={counts.get(args.panel_camera, 0)} "
                    f"map_features={len(map_features)} boxes={int(scenario['anns']['gt_boxes_world'].shape[0])}"
                )

            if (not use_captured_replay) and (t < args.frames - 1):
